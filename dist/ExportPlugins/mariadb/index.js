        ...
        Object.entries(spec.schema).forEach((schema) => {
            ...
            if (schemaSpec.tables) {
                Object.entries(schemaSpec.tables).forEach((table) => {
                    ...
                    if (!(tableSpec.foreignkeys))
                        return;
                    Object.entries(tableSpec.foreignkeys).forEach((fk) => {
                        const [fkName, fkSpec] = fk;
                        if (!(schemaSpec.tables))
                            return;
                        // Check that length of columns matches referenceColumns
                        if (!(fkSpec.columns.length !== fkSpec.referenceColumns.length)) {
                            throw new Error('Number of columns do not match up between child and parent '
                        ...
                        fkSpec.columns.forEach((columnName) => {
                            if (!(columnName in tableSpec.columns)) {
                                throw new Error(`Foreign key constraint '${fkName}' refers to a non-existent `
                        ...
                        const referenceTableSpec = schemaSpec.tables[fkSpec.referenceTable];
                        // Check that referenceColumns exist in referenceTable
                        fkSpec.referenceColumns.forEach((columnName) => {
                        ...
                        fkSpec.columns.forEach((columnName, columnIndex) => {
                            const columnSpec = tableSpec.columns[columnName];
                            const referenceColumnName = fkSpec.referenceColumns[columnIndex];
                            const referenceColumnSpec = referenceTableSpec.columns[referenceColumnName];
                            if (columnSpec.type !== referenceColumnSpec.type) {
                                throw new Error(`Type mismatch between columns '${schemaName}.${tableName}.${columnName}' `
                            ...
                            if (columnSpec.length !== referenceColumnSpec.length) {
                                throw new Error(`Length mismatch between columns '${schemaName}.${tableName}.${columnName}' `
                                    + `and '${schemaName}.${fkSpec.referenceTable}.${referenceColumnName}' as `
                                    + `used by foreign key constraint '${fkName}'.`);
                            }
